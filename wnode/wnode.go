...
var (
	...
	argNewKey  = flag.Bool("newkey", false, "generate new key instead of read from profile for asymmetric encryption")
...
func processArgs() {
	...
	if len(*argTopic) > 0 {
		...
		x, err := hex.DecodeString(profile.Topic)
		if err != nil {
			utils.Fatalf("Failed to parse the topic: %s", err)
		}
		topic = whisper.BytesToTopic(x)
...
func initialize() {
	...
	if *bootstrapMode {
		...
		*bootstrapMode = true
		...
		fmt.Println("The bootnode ", *argEnode)
	...
	if *asymmetricMode && *argNewKey {
		...
		profile.Key = hex.EncodeToString(asymKey.D.Bytes())
		config.UpdateWhisperNode(profile)
	}else {
		b, err := hex.DecodeString(profile.Key)
		if err != nil {
			utils.Fatalf("Failed to load whisper default key: %s.", err)
		}
		...
		if err != nil {
			utils.Fatalf("Failed to load default key", err)
		}
	}
...
func configureNode() {
	...
	// If in asymmetricMode, generate symKey for broadcasting
	if *asymmetricMode {
		symPass = "usechain@2017"
...
/**
 * For point to point communication, determine the destincation public key
 * For msg broadcasting, leave the destpub for nil
**/
func SendMsg(payload []byte, destpub *ecdsa.PublicKey) common.Hash {
